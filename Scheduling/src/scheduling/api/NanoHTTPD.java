...
public class NanoHTTPD {
    ...
    public Response getinfo(String uri, Properties header) {
        if (uri.startsWith("room/")) {
            return getRoomSchedule(uri.substring(5, uri.length()), header);
        }
        if (uri.startsWith("student/")) {
            return getStudentSchedule(uri.substring(8, uri.length()), header);
        }
    ...
    public Response getTeacherSchedule(String uri, Properties header) {
        ...
        return new Response(HTTP_BADREQUEST, "no teacher with that name");
    }
    public Response getStudentSchedule(String uri, Properties header) {
        ...
        return new Response(HTTP_BADREQUEST, "no student with that name");
    }
    public Response getRoomSchedule(String uri, Properties header) {
        Room room = Room.getRoom(Integer.parseInt(uri));
        if (room == null) {
            return new Response(HTTP_BADREQUEST, "Room don't exist bro");
        }
        return new Response(HTTP_OK, room.getinfo(Scheduling.getSchedule()));
    }
    /**
    ...
    public class Response {
        ...
        public Response(String status, String txt) {
            this(status, MIME_PLAINTEXT, txt);
        }
        /**
         * Basic constructor.
         */
     ...
     *
     * public static void main1(String[] args) { myOut.println("NanoHTTPD 1.27
     * (C) 2001,2005-2013 Jarno Elonen and (C) 2010 Konstantinos Togias\n" +
     * "(Command line options: [-p port] [-d root-dir] [--licence])\n"); //
     * Defaults int port = 80; File wwwroot = new File(".").getAbsoluteFile();
     * // Show licence if requested for (int i = 0; i < args.length; ++i) { if
     * (args[i].equalsIgnoreCase("-p")) { port = Integer.parseInt(args[i + 1]);
     * } else if (args[i].equalsIgnoreCase("-d")) { wwwroot = new File(args[i +
     * 1]).getAbsoluteFile(); } else if
     * (args[i].toLowerCase().endsWith("licence")) { myOut.println(LICENCE +
     * "\n"); break; } } try { new NanoHTTPD(port); } catch (IOException ioe) {
     ...
     * catch (Throwable t) { } } /** Handles one session, i.e. parses the HTTP
